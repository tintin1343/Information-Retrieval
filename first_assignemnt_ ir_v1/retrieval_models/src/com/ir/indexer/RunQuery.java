...
public class RunQuery {
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			String[] stop_words_custom = getStopWords().split(" ");
			...
			for (List<String> query : final_query) {
				...
				for (String q : query) {
					...
					if (y > 0) {
						...
						Okapi(client, rankTerm, q, avg_doc_length);
					}
					y++;
				}
	...
	private static void writeToFile(List<List<String>> resultOkapi) {
		...
		try {
			...
			File file = new File("C:/Users/Nitin/okapi-ir-v2-1.txt");
	...
	public static void Okapi(Client client,/* Map<String, Integer> results, */
	...
	private static String getStopWords() {
		...
		try {
			if (stop_words.isFile()) {
				...
				stop_words_list.append("discuss").append(" ")
						.append("identify").append(" ").append("report")
						.append(" ").append("include").append(" ")
						.append("predict").append(" ").append("cite")
		...
		return stop_words_list.toString();
	}
	...
	public static Map<String, Integer> queryTF(Client client, QueryBuilder qb,
		...
		/*while (true) {*/
			while (true) {
			for (SearchHit hit : scrollResp.getHits().getHits()) { //totalhits
