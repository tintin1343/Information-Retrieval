...
public class LaplaceSmoothing {
	public static void main(String[] args) throws IOException {
		...
		/*	Config config = new Config("./configs/config.properties");
			...
			// starts client
			/*
			 * Node node = nodeBuilder().client(true).node(); Client client =
			 * node.client();
			 */
			...
			for (List<String> query : final_query) {
				...
				for ( int i=1; i<query.size();i++) {
					/* Method to calculate tfs for each term in query */
						...
						tfMap = laplaceSmoothing(client, rankTerm, query.get(i).toLowerCase(),
						...
						for (Map.Entry<String, Integer> term : tfMap.entrySet()) {
							...
							if (queryTFList.get(term.getKey()) == null) {
								...
								queryTFList.put(
										term.getKey(),
										queryTFList.get(term.getKey())+" "+term.getValue().toString());
				...
				for (Map.Entry<String, String> d : queryTFList.entrySet()) {
					...
					rankTerm.put(d.getKey(), laplacePerTerm(
			...
			w.writeToFile(resultOkapi,"Laplace-Final-1.txt");
			// node.close();
	...
	private static void writeToFile(List<List<String>> resultOkapi) {
		...
		try {
			...
			File file = new File("C:/Users/Nitin/laplaceSmoothing-1.txt");
	...
	public static Map<String, Integer> laplaceSmoothing(Client client,
		...
		/*for (Map.Entry<String, Integer> term : results.entrySet()) {
			...
			if (rankTerm.get(term.getKey()) != null) {
				rankTerm.put(
						term.getKey(),
						(rankTerm.get(term.getKey()) + laplacePerTerm(
								term.getValue(), stats1.getTotal(),
								avg_doc_length, idf, vocabSize)));
			} else {
				rankTerm.put(
						term.getKey(),
						laplacePerTerm(term.getValue(), stats1.getTotal(),
								avg_doc_length, idf, vocabSize));
			}
		}
*/
	...
	public static double laplacePerTerm(String termFreq, double docLen,
		...
		/*if(termFs.length <= queryWords){
			...
			for(int j=0;j<(queryWords-termFs.length); j++){
	...
	private static XContentBuilder getTF(String matchField, String matchValue)
			throws IOException {
		XContentBuilder builder = XContentFactory.jsonBuilder();
		builder.startObject().startObject("query").startObject("match")
				.field(matchField, matchValue).endObject().endObject()
